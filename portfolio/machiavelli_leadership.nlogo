...
to determine-behavior
  set active? ((grievance - (risk-aversion + fear-factor) * estimated-arrest-probability) - fear-factor) > threshold + generosity    ;added fear-factor - turtle's risk aversion increases along with fear-factor, and vice-versa
end

to-report grievance
   if generosity >= 0 and reputation >= 0
   [report perceived-hardship * (1  - (generosity - (generosity ^ 2) + reputation))]       ;percieved hardship is the square of generosity - generosity, plus reputation (square allows for a sweet spot)
   if generosity < 0 and reputation < 0
   [ report perceived-hardship * (1  + (generosity + (generosity ^ 2) - reputation)) ]
   if generosity >= 0 and reputation <= 0
   [report perceived-hardship * (1  + (generosity - (generosity ^ 2) - reputation)) ]
   if generosity < 0 and reputation >= 0
   [report perceived-hardship * (1  + (generosity + (generosity ^ 2) + reputation)) ]
...
to enforce
  if any? (agents-on neighborhood) with [ not active?] [
    ;; arrest suspect
    let suspect one-of (agents-on neighborhood) with [ not active?]
    ask suspect [
      set active? false
...
"supportive" 1.0 0 -955883 true "" "plot count agents with [jail-term > 0]"
"active" 1.0 0 -11221820 true "" "plot count agents with [active?]"
...
<experiments>
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="fear-factor">
      <value value="-1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="generosity">
      <value value="0"/>
    </enumeratedValueSet>
